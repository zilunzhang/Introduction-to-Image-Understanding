...
def match(template_key_points, template_descriptor, find_key_points, find_descriptor, threshold):
    ...
    print("find_key_points's len is: {}".format(len(find_key_points)))
    print("template descriptor's shape is: {}".format(template_descriptor.shape))
    print("find descriptor's shape is: {}".format(find_descriptor.shape))
    ...
    print("template_key_points after: {}".format(len(template_key_points)))
    print("find_key_points after: {}".format(len(find_key_points)))
    return template_key_points, find_key_points, match_dis_list


# q2. c
def affine_transformation(match_template_points, match_find_key_points, k):
...
# q2. e
def extract_match_transformation_visualization(RGB_template_image, RGB_target_image):
    ...
    match_template_red_kpt, match_target_red_kpt, red_dis = match\
        (grey_template_kpt, grey_template_descriptor, red_target_kpt, red_target_descriptor, 0.8)
    ...
    match_template_green_kpt, match_target_green_kpt, green_dis = match\
        (grey_template_kpt, grey_template_descriptor, green_target_kpt, green_target_descriptor, 0.8)
    ...
    match_template_blue_kpt, match_target_blue_kpt, blue_dis = match\
        (grey_template_kpt, grey_template_descriptor, blue_target_kpt, blue_target_descriptor, 0.8)
...
if __name__ == '__main__':
    start = datetime.now()
    image_book = cv2.imread("book.jpg")
    RGB_image_book = cv2.cvtColor(image_book, cv2.COLOR_BGR2RGB)
    gray_image_book = cv2.cvtColor(RGB_image_book, cv2.COLOR_RGB2GRAY)
    print("image book's size is: {}".format(gray_image_book.shape))
    image_find_book = cv2.imread("findBook.jpg")
    RGB_image_find_book = cv2.cvtColor(image_find_book, cv2.COLOR_BGR2RGB)
    gray_image_find_book = cv2.cvtColor(RGB_image_find_book, cv2.COLOR_RGB2GRAY)
    print("image find book's size is: {}".format(gray_image_find_book.shape))
    feature_book_key_pts, book_descriptor = extract_feature(gray_image_book)
    feature_find_book_key_pts, find_book_descriptor = extract_feature(gray_image_find_book)
    match_template_points, match_find_key_points, distance_list = \
        match(feature_book_key_pts, book_descriptor, feature_find_book_key_pts, find_book_descriptor, 0.8)
    transform_matrix, match_template_points, match_find_key_points = affine_transformation(match_template_points, match_find_key_points, 3)
    visualization(gray_image_book, match_template_points, gray_image_find_book, match_find_key_points, transform_matrix)

    color_template = cv2.imread("colourTemplate.png")
    # color_template = cv2.imread("book.jpg")
    RGB_color_template = cv2.cvtColor(color_template, cv2.COLOR_BGR2RGB)
    color_search = cv2.imread("colourSearch.png")
    # color_search = cv2.imread("findBook.jpg")
    RGB_color_search = cv2.cvtColor(color_search, cv2.COLOR_BGR2RGB)
    final_color_template_kpts, final_color_target_kpts = extract_match_transformation_visualization(RGB_color_template, RGB_color_search)
    M_color, match_color_template_points, match_color_find_key_points = affine_transformation(final_color_template_kpts, final_color_target_kpts, 6)
    visualization(RGB_color_template, match_color_template_points, RGB_color_search, match_color_find_key_points, M_color)
