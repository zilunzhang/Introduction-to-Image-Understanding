...
# row, col, ch = RGB_color_target.shape
...
def ransac(template_img, target_img):
    ...
    # find kpts
    template_kpts, template_descriptor = extract_feature(template_img)
    target_kpts, target_descriptor = extract_feature(target_img)
    match_template_kpts, match_target_kpts, match_dis_list = \
        match(template_kpts, template_descriptor, target_kpts, target_descriptor, ratio_threshold)
...
def reconstruct(img_template, img_set):
    ...
    for permutation in index_list:
        ...
        if mssd < best_mssd:
            ...
            if best_mssd == 0 and best_idx[0] == 1:
...
if __name__ == '__main__':
    ...
    # gray_color_template = cv2.cvtColor(RGB_color_template, cv2.COLOR_RGB2GRAY)
    # gray_color_target = cv2.cvtColor(border_img, cv2.COLOR_RGB2GRAY)
    # transformation, mssd, match_template_kpts, match_target_kpts = ransac(gray_color_target, gray_color_template)
    ...
    # plt.show()
    # cv2.imwrite("q2a.jpg", RGB_color_target)
    # images = load_images_from_folder("shredded")
    # color_template = cv2.imread("mugShot.jpg")
    # RGB_color_template = cv2.cvtColor(color_template, cv2.COLOR_BGR2RGB)
    # gray_color_template = cv2.cvtColor(RGB_color_template, cv2.COLOR_RGB2GRAY)
    # best_mssd, best_tran, best_idx = reconstruct(gray_color_template, images)
    best_idx = [4, 0, 3, 2, 5, 1]
